...
for k = 451:539;
        pmin_1(k)=((Pa*10^5)*0.000145)*ed1^n01-pmin_1(450)*(k-350)/28;
        T_value_1(k)=(Tc*((abs(pmin_1(k))/Pc_1)))+273;
end
for k = 540:580;
         SE1 = ((mS(k)*(1+cosd(A))/2)+L-sqrt((L^2)+((mS(k)*sind(A))^2)/4));
     ...
     if k == 540;
         P_min=((Pa*10^5)*0.000145)*ed1^n01;
         pmin_1(k) = pmin_1(450)-pmin_1(450)*(k-350)/30;
     else
...
path_P = fullfile(pwd,'..','output','Program','simulate-input','data_P_loi_8.dat');
path_T = fullfile(pwd,'..','output','Program','simulate-input','data_T_loi_8.dat');
cla;
