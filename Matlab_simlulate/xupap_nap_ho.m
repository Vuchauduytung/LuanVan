...
for k = 1:9;
    ...
    if mod(k,2);
        pmin_1(k)=((Pa*10^5)*0.000145)*ed1^n01+5;
    else 
        pmin_1(k)=((Pa*10^5)*0.000145)*ed1^n01-5;
    end
        pmin_1(9)=((Pa*10^5)*0.000145)*ed1^n01;
...
for k = 10:49;
    ...
    if k < 12;
        ...
        pmin_1(k)=pmin_1(10)-P_min+((Pa*10^5)*0.000145)*ed1^n01;
    end
    T_value_1(k)=(Tc*((abs(pmin_1(k))/Pc_1)))+273;
end
for k = 50:210;
        ...
        n01 = (8.314/(19.806+0.002095*T*(ed1^(p-1)+1))+1) ;
    if mod(k,2);
        pmin_1(k)=((Pn*10^5)*0.000145)*ed1^n01+1;
    else 
        pmin_1(k)=((Pn*10^5)*0.000145)*ed1^n01-1;
    end
        pmin_1(210)=((Pn*10^5)*0.000145)*ed1^n01;
        T_value_1(k)=(Tc*((abs(pmin_1(k))/Pc_1)))+273;
end
for k = 211:270;
        pmin_1(k)=(((Pn*10^5)*0.000145)*ed1^n01 + pmin_1(20)*(k-180)/10)*0.7-20;
        T_value_1(k)=(Tc*((abs(pmin_1(k))/Pc_1)))+273;
end
for k = 271:360;
        SE1 = ((mS(k)*(1+cosd(A))/2)+L-sqrt((L^2)+((mS(k)*sind(A))^2)/4));
...
path_P = fullfile(pwd,'..','output','Program','simulate-input','data_P_loi_6.dat');
path_T = fullfile(pwd,'..','output','Program','simulate-input','data_T_loi_6.dat');
cla;
